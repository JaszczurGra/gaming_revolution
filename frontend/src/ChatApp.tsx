...
// Default to the backend on the same host the page was loaded from (port 8000 unless
// VITE_API_PORT overrides it), so this also works when the page is opened from another device,
// e.g. over Tailscale, where "localhost" would otherwise mean that other device rather than this
// one. Override the whole URL with VITE_API_BASE instead if host and port both differ.
...
export default function ChatApp() {
  ...
  useEffect(() => {
    if (!gameId) return
    getGame(gameId).then((g) => setGameName(g?.name ?? null))
  }, [gameId])
    ...
    <div className={`app${isMobile ? ' app-mobile' : ''}`}>
      <header className="app-header">
        <div className="app-header-left">
          <Link to={gameId ? `/game/${gameId}` : '/dashboard'} className="back-link" title="Back to library">
            &larr;
          </Link>
          <h1>{gameName ? `${gameName} Assistant` : 'Board Game Assistant'}</h1>
        </div>
        <div className="app-header-right">
          <button className="theme-toggle-btn" onClick={toggleTheme} title="Toggle light/dark theme">
            {theme === 'dark' ? '☀️' : '🌙'}
          </button>
          {!isMobile && <span className="user-chip">{user?.isAnonymous ? 'Guest' : user?.email}</span>}
          <button className="reset-btn" onClick={resetGame} title="Start a new game">
            New game
          </button>
          {!isMobile && (
            <button className="reset-btn" onClick={handleLogout} title="Log out">
              Log out
            </button>
          )}
        </div>

...
To edit content, edit the chunk markdown and rerun. To add a chunk, copy an existing file's front-matter shape — `id`, `title`, `section`, `authority`, `tags` are required.

## Testing suggestions

Worth checking before you ship:

1. "Can I build a settlement next to my own settlement?" → should cite the Distance Rule and say no.
2. "Can I trade 2 wool for 1 wool?" → no, same-resource trades are forbidden.
3. Photo with a Distance Rule violation → should name both intersections and offer legal alternatives.
4. Photo, then "what's my best move?" → should ask for the hand before advising.
5. "What happens in Seafarers when..." → should decline as out of scope.
6. Photo with a hex deliberately obscured → should notice the terrain distribution check fails and ask for a reshoot.

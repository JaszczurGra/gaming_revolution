...
import { Camera, Send, ArrowLeft, Loader2, Image as ImageIcon, X, Play, Square, Eye, EyeOff, Sparkles } from 'lucide-react';
...
export default function Chatbot() {
  ...
  const [history, setHistory] = useState<{ role: string; content: string; imageUrl?: string | null }[]>([]);
  ...
  const handleLiveCapture = useCallback(async () => {
    if (!webcamRef.current || !isLiveMode || isCapturingLive) return;
    ...
    // Create temporary history so we don't block the UI while it thinks
    const userMessage = "(Live Observation Frame)";
    ...
    try {
      const res = await fetch('/api/gemini/chat', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: "Analyze the current board state and give me a brief, tactical hint or note any rules being broken. Keep it very short.",
          gameName: game?.name,
          gameNotes: game?.notes,
          pdfUrl: game?.pdfUrl,
      ...
      const decoder = new TextDecoder();
      let streamedText = '';
      setHistory(prev => [...prev, { role: 'model', content: "👀 Live observation: " }]);
      ...
      while (true) {
        ...
        setHistory(prev => {
          const newH = [...prev];
          newH[newH.length - 1].content = "👀 Live observation: " + streamedText;
          return newH;
        });
      }
    } catch (error) {
  ...
  }, [game, history, isLiveMode, isCapturingLive]);
  ...
  const handleSend = async () => {
    ...
    const userMessage = input.trim() || (imagePreview ? "Here is an image of the board state." : "");
    const base64ToSend = imagePreview || undefined;
    
    const newHistory = [...history, { role: 'user', content: userMessage, imageUrl: base64ToSend }];
    setHistory(newHistory);
    
    // Clear input bar and image preview immediately to prevent lingering
    setInput('');
    setImagePreview(null);
    setLoading(true);
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-5rem)] md:h-[calc(100vh-8rem)] md:max-h-[800px] max-w-4xl mx-auto bg-white dark:bg-slate-800 md:rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
      <div className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700 p-4 shrink-0 flex items-center justify-between gap-2 flex-wrap md:flex-nowrap">
        <div className="flex items-center gap-4">
          <Link to={`/game/${gameId}`} className="text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h2 className="font-bold text-slate-900 dark:text-white line-clamp-1">{game.name} Assistant</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">Ask rules, tactics, or show the board</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            {isLiveMode ? (
              <><Square className="w-3 h-3 md:w-4 md:h-4 fill-current" /> <span className="hidden sm:inline">Stop Live Help</span><span className="sm:hidden">Stop</span></>
            ) : (
              <><Play className="w-3 h-3 md:w-4 md:h-4 fill-current" /> <span className="hidden sm:inline">Start Live Help</span><span className="sm:hidden">Start Live</span></>
            )}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-slate-50/50 dark:bg-slate-900/50">
        {history.length === 0 && (
          <div className="text-center text-slate-500 dark:text-slate-400 mt-10">
            ...
            <p className="text-sm mt-1">Start by asking a question about {game.name}.</p>
          </div>
        )}
        {history.map((msg, i) => {
      ...
      <div className="bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 p-3 md:p-4 shrink-0 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-1 md:gap-2">
          ...
          <input 
            ...
            placeholder={isColorblind ? "Ask about colors, pieces, or rules..." : "Ask a question..."}
            className="flex-1 w-0 min-w-0 bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white border-none rounded-xl px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-800 transition-all outline-none text-sm md:text-base font-medium"
          />
          <button 
            onClick={handleSend}
            disabled={loading || (!input.trim() && !imagePreview)}
            className="bg-indigo-600 text-white p-2.5 md:p-3 rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-colors shrink-0"
          >
